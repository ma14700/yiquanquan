        ...
        <mt-tab-container v-model="selected">
            <mt-tab-container-item id="0">
                ...
                <div v-else class="mygoods" v-for="(group,index) in myorder" :key="index" >
                    ...
                    <div class="goods-wraper" v-for="item in group.orderItemList" :key="item.id" @click="goDetail(item,group)">
                        <div class="goods-img">
                        ...
                        <div class="goods-price">
                            ...
                            <p class="oldprice">￥{{item.itemPrice | two}}</p>
                            <p class="num">x{{item.itemGoodNum}}</p>
                        </div>
                    </div>
                    <div class="mystatus">
                        <p class="total-num">共<i v-text="group.orderType == 1 ? '1': myorder[index].orderNum"></i>件 ,  <i v-text="group.orderType == 1 ? '需付定金：¥'+group.orderPromiseMoney:'需付款：¥'+group.orderTotalFee"></i></p>
                        <!-- <span>确认发货</span> -->
                        <span class="waitting" @click="goPay(group)" >去支付</span>
                    </div>
                </div>
            ...
            <mt-tab-container-item id="1">
                ...
                <div class="mygoods" v-for="(group,index) in waitPay" :key="index">
                    <p class="order-id">
                        <!-- {{group.orderNo}} -->
                        <span>{{group.orderStatus}}</span>
                    </p>
                    ...
                    <div class="mystatus">
                        <p class="total-num">共<i v-text="group.orderType == 1 ? '1': waitPay[index].orderNum"></i>件 ,  <i v-text="group.orderType == 1 ? '需付定金：¥'+group.orderPromiseMoney:'需付款：¥'+group.orderTotalFee"></i></p>
            ...
            <mt-tab-container-item id="2">
                ...
                <div class="mygoods" v-for="(group,index) in waitGoods" :key="index" >
                    <p class="order-id">
                        <!-- {{group.orderNo}} -->
                        <span>{{group.orderStatus}}</span>
                    </p>
                    ...
                    <div class="mystatus">
                        <p class="total-num">共<i v-text="group.orderType == 1 ? '1': waitGoods[index].orderNum"></i>件 ,  <i v-text="group.orderType == 1 ? '需付定金：¥'+group.orderPromiseMoney:'需付款：¥'+group.orderTotalFee"></i></p>
            ...
            <mt-tab-container-item id="7">
                ...
                <div class="mygoods" v-for="(group,index) in waitEvaluated" :key="index">
                    <p class="order-id">
                        <!-- {{group.orderNo}} -->
                        <span>{{group.orderStatus}}</span>
                    </p>
                    ...
                    <div class="mystatus">
                        <p class="total-num">共<i v-text="group.orderType == 1 ? '1': waitEvaluated[index].orderNum"></i>件 ,  <i v-text="group.orderType == 1 ? '需付定金：¥'+group.orderPromiseMoney:'需付款：¥'+group.orderTotalFee"></i></p>
                        <span class="waitting" @click="goPay(group)">去支付</span>
                    </div>
                </div>
            </mt-tab-container-item>
