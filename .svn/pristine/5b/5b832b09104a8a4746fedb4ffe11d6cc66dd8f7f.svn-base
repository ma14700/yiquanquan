        ...
        <li v-for="(good, index) in cartList" class="good-wraper" :key="index">
          ...
          <div >
            ...
            <div class="shopedit" :class="[delShow?'delete-show':'delete-hide']">
              <div class="count">
                <button @click.stop="reduceQuentity(good,-1,index)">-</button>
                <input type="text" :value="good.cartNum" v-model="good.cartNum" v-on:blur="freeChangeQuentity(good)">
                <button @click.stop="addQuentity(good,1)">+</button>
              </div>
