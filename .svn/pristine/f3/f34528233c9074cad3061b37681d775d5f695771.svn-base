            ...
            <mt-tab-container-item id="1">
                 <div class="main-body" :style="{'-webkit-overflow-scrolling': scrollMode}">  
                    <v-loadmore  :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore"> 
                        <ul class="clearfix">
                        <li  v-for="(item,index) in shopList" :class="[test?'lump-wraper':'item-wraper']">
                            <div class="img-box">
                                <img v-lazy.container="item.goodFirstPicUrl" alt="">
                            </div>
                            <div class="content">
                                <p class="title" ref="title"  @click="goShopdetail(item.id)">{{item.goodName}}</p>
                                <p class="price"  @click="goShopdetail(item.id)">¥{{item.goodSalePrice | two}}
